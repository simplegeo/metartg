...
def get_available_graphs(host):
    ...
    if not rrdpath.startswith(RRDS):
        bottle.abort(400, 'That\'s not right...')
    ...
    for filename in os.listdir(GRAPHDEFS):
        ...
        try:
            ...
            bottle.abort(500, 'Error parsing graph definition file %s: %s' % (filename, str(e)))
    ...
    return dumps(available)

@bottle.get('/graph/')
def get_available_hosts():
    return dumps(os.listdir(RRDS))
